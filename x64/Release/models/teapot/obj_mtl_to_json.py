...
def parseGeometry(file, hasMaterials):    
    ...
    for line in open(file, 'r').readlines():
        nLine = nLine + 1
        try:
            if line.startswith('usemtl') and hasMaterials:            
                    words = line.split()
                    if (len(words) == 2):
                ...
                vertices = OBJECTS[OBJECT_NAME]['vertices']             
                normals = OBJECTS['normals']               
                ...
                indices     = OBJECTS[OBJECT_NAME]['group'][GROUP_NAME]['indices']          
                normals_idx = OBJECTS[OBJECT_NAME]['group'][GROUP_NAME]['normals_idx']    
            ...
            elif location == LOC_GROUP:                                     
                if line.startswith('f '):
                    f = line[1:len(line)].split()
                    pl = len(f)
                    if (pl == 3):                                         #for WebGL: faces are triangles
...
if __name__ == '__main__':
   if (len(sys.argv) == 1):
        print 'ERROR -- Please type: python obj_mtl_to_json.py *.obj *.mtl'
        sys.exit(0)
   FILE = sys.argv[1]     
